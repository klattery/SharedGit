...
data {
  ...
  matrix[I,P_cov] i_cov; // Data for individuals I (resp covariates)
...
transformed parameters {
  matrix[P, I] beta_ind;
  {
    ...
    if (P_cov == 0){
      beta_ind = rep_matrix(alpha, I) + (cov_chol * z); // P x I unconstrained
    } else beta_ind = rep_matrix(alpha, I) + (i_cov_load * i_covt) + (cov_chol * z); // P x I unconstrained
    if (con_n >0){
